...
class ImageEncodingMissingError(ValueError):
    """Image has no encoding value. Happens if image message is empty"""
    def __init__(self, msg=None):
        if msg is None:
            # Set some default useful error message
            msg = "Image encoding is missing. Probably an empty image message"
        super(ImageEncodingMissingError, self).__init__(msg)
...
def numpy_to_image(arr, encoding, header):
    # type: (np.ndarray, Any, Any)-> MsgImage
    if not encoding in name_to_dtypes:
        raise TypeError('Unrecognized encoding {}'.format(encoding))

    im = MsgImage(encoding=encoding, header=header)
