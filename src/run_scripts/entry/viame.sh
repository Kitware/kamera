...
if [[ $(redis-cli --raw -h $REDIS_HOST get /debug/rebuild ) == "true" ]]; then
  ...
  colcon build --packages-select sprokit_adapters
  if [[ $? -ne 0 ]]; then
...
printf "
\$ exec ros2 launch sprokit_adapters sprokit_detector_fusion_adapter.launch.xml \
                    kwiver:=${WS_DEVEL} \
                    ...
                    "
# Detection output dirs are read from the environment by the pipelines
export DETECTION_CSV_DIR
export IMAGE_LIST_DIR

RESPAWN=$([[ "${NORESPAWN:-false}" == "true" ]] && echo false || echo true)
exec ros2 launch sprokit_adapters sprokit_detector_fusion_adapter.launch.xml \
                    system_name:=${NODE_HOSTNAME} \
                    detector_node:=detector \
                    detection_pipefile:="${PIPEFILE}" \
                    ...
                    ocv_num_threads:=4 \
                    sync_q_size:=${SYNC_Q_SIZE} \
                    respawn:=${RESPAWN} 2> >(tee -a /tmp/roslaunch_err.log >&2)
