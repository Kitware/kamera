...
do
    ...
    fdfind . $source_dir --exclude '*.{IIQ,iiq,txt,csv,dat,flight_data_mounted}' --changed-before 10s --type f --exec echo {} | \
    ...
    fdfind . $source_dir --exclude '*.{IIQ,iiq,json,tif,jpg,flight_data_mounted}' --changed-before 1day --type f --exec echo {} | \
        # remove far left dir for rsync
        cut -d'/' -f4- | \
        # cap max argument length for rsync
        head -n 1000 | \
