...
do
  if [[ -z "${!VNAME}" || "${!VNAME}" == 'null' ]]; then
    printf "<launchpod.sh!> Unable to determine $VNAME: ${CAM_FOV}. Check user-config\n"
    exit 1
  fi
done
...
docker compose -f ${KAM_REPO_DIR}/compose/process.yml -f ${KAM_REPO_DIR}/compose/cam.yml "${ARGS[@]}" &
PROC_STAT=$!
set +a
