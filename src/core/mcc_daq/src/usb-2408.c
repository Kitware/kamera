...
/* Commands and Report ID for the USB 2408 */
...
int usbAInScanRead_USB2408(libusb_device_handle *udev, uint16_t count, uint8_t nChan, int32_t *data, int options)
{
  ...
  if (transferred != nbytes) {
    fprintf(stderr, "usbAInScanRead_USB2408: number of bytes transferred = %d, nbytes = %d\n", transferred, nbytes);
    return ret;
  }
...
double volts_USB2408_d(const int gain, double value)
{
    double volt = 0.0;
