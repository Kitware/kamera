...
class _TestPigeonhole(object):
        ...
        while attempt < retries:
            with self._cond:
                ...
                if value in self._deque:
                    logging.debug('Pigeonhole contains {}'.format(value))
                    return True
                logging.debug('Pigeonhole waiting for {}'.format(value))
                self._cond.wait(timeout=timeout)
                attempt += 1
    ...
    def absent(self, value):
        ...
        while True:
            with self._cond:
                if value not in self._deque:
                    logging.debug('Pigeonhole lacks {}'.format(value))
                    return True
                logging.debug('Pigeonhole waiting on lack of {}'.format(value))
                self._cond.wait()
