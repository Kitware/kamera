...
class CamToCamTform(object):
    ...
    def __init__(self, src_cm, dst_cm):
        if (
            src_cm.platform_pose_provider != dst_cm.platform_pose_provider
            and not isinstance(src_cm.platform_pose_provider, PlatformPoseFixed)
            and not isinstance(dst_cm.platform_pose_provider, PlatformPoseFixed)
        ):
            raise Exception(
                "src_cm and dst_cm must have the same "
...
def ray_intersect_plane(plane_point, plane_normal, ray_pos, ray_dir, epsilon=1e-6):
...
class Camera(object):
    ...
    def __str__(self):
        ...
        string.append(
            "".join(["platform_pose_provider: ", repr(self._platform_pose_provider)])
        )
    ...
    def unproject_to_llh(self, points, t=None, cov=None):
        ...
        for i in range(points.shape[1]):
            ...
            if cov is not None:
                ...
                # system
                rpoints = np.random.multivariate_normal(points[:, i], cov[i], 20)
                ...
                rpoints = rpoints[ind]
                enu_pts = [self.unproject_to_depth(_, t).ravel() for _ in rpoints]
        ...
        pts = np.vstack(
            [
                np.hstack(
                    [
                        x,
                        np.full(len(y), self.width, dtype=np.float64),
                        x[::-1],
                        np.zeros(len(y)),
                    ]
                ),
                np.hstack(
                    [
                        np.zeros(xn),
                        y,
                        np.full(xn, self.height, dtype=np.float64),
                        y[::-1],
                    ]
                ),
            ]
...
class StandardCamera(Camera):
    ...
    def __init__(
        self, width, height, K, dist, cam_pos, cam_quat, platform_pose_provider=None
    ):
        """
        See additional documentation from base class above.

        """
        super(StandardCamera, self).__init__(width, height, platform_pose_provider)
    ...
    def __str__(self):
        ...
        string.append(
            "".join(["distortion_coefficients: ", repr(tuple(self._dist)), "\n"])
        )
        string.append(
            "".join(["camera_quaternion: ", repr(tuple(self._cam_quat)), "\n"])
        )
    ...
    def project(self, points, t=None):
        ...
        tvec = pose_mat[:3, 3]
        im_pts = cv2.projectPoints(points.T, rvec, tvec, self._K, self._dist)[0]
    ...
    def unproject(self, points, t=None, normalize_ray_dir=True):
        ...
        ray_dir0 = cv2.undistortPoints(
            np.expand_dims(points.T, 1), self._K, self._dist, R=None
        )
...
class RollingShutterCamera(StandardCamera):
    ...
    def __init__(
        ...
        """
        super(RollingShutterCamera, self).__init__(
            width, height, K, dist, cam_pos, cam_quat, platform_pose_provider
        )
    ...
    def project(self, points, t=None):
        ...
        if False:
            ...
            im_pts_list = [
                proj_fun(points, t + alpha * self.shutter_roll_time).T
                for alpha in alphas
            ]
            ...
            im_pts1 = np.hstack(
                [
                    np.take_along_axis(im_pts_list[0], ind1, axis=1),
                    np.take_along_axis(im_pts_list[1], ind1, axis=1),
                ]
            ).T

            im_pts2 = np.hstack(
                [
                    np.take_along_axis(im_pts_list[0], ind2, axis=1),
                    np.take_along_axis(im_pts_list[1], ind2, axis=1),
                ]
            ).T
    ...
    def unproject(self, points, t, normalize_ray_dir=True):
        ...
        ret = [
            super(RollingShutterCamera, self).unproject(points[:, i : i + 1], ts_[i])
    ...
    def save_to_file(self, filename, save_depth_viz=True):
        ...
        if self.depth_map is not None:
            im = PIL.Image.fromarray(
                self.depth_map.astype(np.float32), mode="F"
            )  # float32
    ...
    def _unproject_to_depth(self, points, depth_map, t=None):
        ...
        for i in range(points.shape[1]):
            ...
            if ix < 0 or iy < 0 or ix >= self.width or iy >= self.height:
                ...
                print(y == self.height)
                raise ValueError(
                    "Coordinates (%0.1f,%0.f) are outside the "
                    "%ix%i image" % (x, y, self.width, self.height)
                )
...
class GeoStaticCamera(DepthCamera):
    ...
    def __init__(
        ...
        super(GeoStaticCamera, self).__init__(
            width=width,
            height=height,
            K=K,
            dist=dist,
    ...
    def load_from_file(cls, filename, platform_pose_provider=None):
        ...
        return cls(width, height, K, dist, depth_map, latitude, longitude, altitude, R)
    ...
    def save_to_file(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            f.write(
                "".join(
                    [
                        "# Rotation matrix mapping vectors defined in an "
            ...
            f.write(
                "".join(
                    [
                        "# Location of the camera's center of "
    ...
    def project(self, points, t=None):
        ...
        # Project rays into camera coordinate system.
        im_pts = cv2.projectPoints(points.T, self._rvec, self._tvec, self.K, self.dist)[
            0
        ]
    ...
    def unproject(self, points, t=None, normalize_ray_dir=True):
        ...
        ray_dir = np.ones((3, points.shape[1]), dtype=points.dtype)
        ray_dir0 = cv2.undistortPoints(
            np.expand_dims(points.T, 1), self.K, self.dist, R=None
        )
        ray_dir[:2] = np.squeeze(ray_dir0, 1).T
...
class MapCamera(Camera):
    ...
    def __init__(self, base_layer):
        super(MapCamera, self).__init__(width=base_layer.res_x, height=base_layer.res_y)
        self.base_layer = base_layer

    def project(self, points, t=None):
        return np.array(
            [self.base_layer.meters_to_raster(point) for point in points.T]
        ).T
