...
def _text_page(pdf: PdfPages, title: str, body: str) -> None:
    ...
    fig.text(0.06, 0.88, body, fontsize=9.5, va="top", family="monospace", linespacing=1.4)
    pdf.savefig(fig)
    plt.close(fig)


def _table_page(pdf: PdfPages, title: str, header: list[str], rows: list[list], widths=None) -> None:
    fig, ax = plt.subplots(figsize=PAGE)
    ax.axis("off")
    ax.set_title(title, fontsize=15, weight="bold", loc="left", pad=20)
    table = ax.table(cellText=rows, colLabels=header, loc="upper center", cellLoc="center", colWidths=widths)
...
def camera_page(pdf: PdfPages, cal: RigCalibration) -> None:
    header = ["camera", "size", "fx", "fy", "cx", "cy", "k1", "k2", "p1", "p2", "frames", "rms px", "ifov deg"]
    rows = []
    for name in sorted(cal.cameras):
        c = cal.cameras[name]
        rows.append([name, f"{c.width}x{c.height}", f"{c.K[0, 0]:.1f}", f"{c.K[1, 1]:.1f}", f"{c.K[0, 2]:.1f}", f"{c.K[1, 2]:.1f}",
                     *[f"{v:.5f}" for v in c.dist], c.frames, f"{c.reproj_rms_px:.2f}", f"{np.degrees(1 / c.K[0, 0]):.5f}"])
...
def rig_page(pdf: PdfPages, cal: RigCalibration) -> None:
    ...
    fig = plt.figure(figsize=PAGE)
    fig.suptitle(f"Rig geometry relative to {cal.reference}", fontsize=15, weight="bold", x=0.06, ha="left")
    ...
    for i, name in enumerate(sorted(cal.cameras)):
        ...
        ax3.quiver(0, 0, 0, *z, length=1.0, label=name, arrow_length_ratio=0.08, color=f"C{i}")
    ax3.set_xlim(-1, 1); ax3.set_ylim(-1, 1); ax3.set_zlim(0, 1)
    ax3.set_xlabel("rig x"); ax3.set_ylabel("rig y"); ax3.set_zlabel("rig z (optical)")
...
def boresight_page(pdf: PdfPages, cal: RigCalibration) -> None:
    ...
    fig.suptitle(f"INS boresight: ins_from_rig euler ZYX = ({e[0]:.4f}, {e[1]:.4f}, {e[2]:.4f}) deg, lever arm = "
                 f"({cal.lever_arm_m[0]:.2f}, {cal.lever_arm_m[1]:.2f}, {cal.lever_arm_m[2]:.2f}) m; "
                 f"{keep.sum()} frames, {(~keep).sum()} rejected", fontsize=10, weight="bold")
    for i, lbl in enumerate("xyz"):
        axes[0, 0].plot(t[keep], res[keep, i], ".", ms=2, label=f"rot {lbl}")
        axes[1, 0].plot(t[keep], pos[keep, i], ".", ms=2, label=f"pos {lbl}")
    axes[0, 0].set(title="per-frame boresight residual (deg, rig axes)", xlabel="s since first frame"); axes[0, 0].legend(fontsize=7)
    axes[1, 0].set(title="rig origin vs INS minus lever arm (m, body axes)", xlabel="s since first frame"); axes[1, 0].legend(fontsize=7)
    axes[0, 1].hist(mag, bins=50, color="gray")
    axes[0, 1].set(title=f"residual magnitude: median {np.median(mag):.3f}, p90 {np.percentile(mag, 90):.3f} deg", xlabel="deg")
    axes[1, 1].hist(cal.ins_gap_s * 1000, bins=40, color="gray")
    axes[1, 1].set(title=f"INS sample staleness: median {np.median(cal.ins_gap_s) * 1000:.1f} ms", xlabel="ms")
...
def homography_page(pdf: PdfPages, pair: dict) -> None:
    ...
    for i, (key, title) in enumerate([("warped_img", f"{pair['left']} warped into {pair['right']}"), ("right_img", pair["right"]), ("overlay_img", "overlay (magenta/green)")]):
        ...
        ax.imshow(pair[key]); ax.set_title(title, fontsize=9); ax.axis("off")
...
def write_report(path: str, cal: RigCalibration, pairs: list[dict], notes: str = "") -> None:
    with PdfPages(path) as pdf:
        _text_page(pdf, f"KAMERA rig calibration: {cal.rig}", textwrap.dedent(f"""\
            flight:            {cal.flight}
