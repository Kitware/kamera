...
def extract_features(db_path: str, image_dir: str, names: dict, focal_px: dict, max_image_size: int, num_features: int) -> None:
    ...
    for camera in sorted({c for c, _ in names.values()}):
        ...
        reader = pc.ImageReaderOptions(camera_model=CAMERA_MODEL, camera_params=f"{f},{f},{w / 2},{h / 2},0,0,0,0")
        # Each thread decodes a full-resolution image, so large sensors get fewer threads.
        opts = pc.FeatureExtractionOptions(max_image_size=max_image_size, use_gpu=pc.has_cuda, num_threads=4 if w * h > 40e6 else 16)
        opts.sift.max_num_features = num_features
        pc.extract_features(db_path, image_dir, image_names=image_names, camera_mode=pc.CameraMode.PER_FOLDER,
                            reader_options=reader, extraction_options=opts, device=device())
...
def write_pose_priors(db_path: str, names: dict, ins, std_m: float) -> None:
    ...
    for image in db.read_all_images():
        prior = pc.PosePrior(position=ins.pose(names[image.name][1])[0], position_covariance=np.eye(3) * std_m**2,
                             coordinate_system=pc.PosePriorCoordinateSystem.CARTESIAN)
        prior.corr_data_id = pc.data_t(pc.sensor_t(pc.SensorType.CAMERA, image.camera_id), image.image_id)
...
def match_features(db_path: str, max_distance_m: float, max_neighbors: int) -> None:
    ...
    pairing = pc.SpatialPairingOptions(max_num_neighbors=max_neighbors, max_distance=max_distance_m, ignore_z=True)
    pc.match_spatial(db_path, matching_options=pc.FeatureMatchingOptions(use_gpu=pc.has_cuda), pairing_options=pairing, device=device())
...
def prune_cross_spectral(db_path: str) -> int:
    ...
    db = pc.Database.open(db_path)
    is_ir = {im.image_id: im.name.split("/")[0].endswith("_ir") for im in db.read_all_images()}
...
def mapping_options(refine_rig: bool) -> pc.IncrementalPipelineOptions:
    ...
    opts = pc.IncrementalPipelineOptions(use_prior_position=True, ba_refine_sensor_from_rig=refine_rig, extract_colors=False)
...
def run_mapping(db_path: str, image_dir: str, out_dir: str) -> dict[int, pc.Reconstruction]:
    """Incremental mapping from scratch with every camera independent (trivial rigs)."""
    shutil.rmtree(out_dir, ignore_errors=True)
    os.makedirs(out_dir)
    return pc.incremental_mapping(db_path, image_dir, out_dir, options=mapping_options(refine_rig=False))


def rig_bundle_adjust(model: pc.Reconstruction, priors: list, refine_intrinsics: bool, max_iterations: int) -> str:
    """Refine rig poses, sensor_from_rig and optionally intrinsics, anchored to the INS position priors."""
    opts = pc.BundleAdjustmentOptions(refine_sensor_from_rig=True, refine_rig_from_world=True, refine_principal_point=False,
                                      refine_focal_length=refine_intrinsics, refine_extra_params=refine_intrinsics, print_summary=False)
    ...
    summary = pc.create_pose_prior_bundle_adjuster(opts, prior_opts, config, priors, model).solve()
    model.update_point_3d_errors()
    return summary.brief_report()


def refine_rig(db_path: str, names: dict, init_dir: str, out_dir: str, max_iterations: int = 200) -> pc.Reconstruction:
    ...
    for refine_intrinsics in (False, True):
        model = pc.triangulate_points(model, db_path, image_dir, out_dir, clear_points=True, options=opts, refine_intrinsics=False)
        print(f"  triangulated {model.num_points3D()} points, {model.compute_mean_reprojection_error():.2f} px", flush=True)
        print(f"  {rig_bundle_adjust(model, priors, refine_intrinsics, max_iterations)}", flush=True)
    model.write(out_dir)
    return model


def load_models(out_dir: str) -> dict[int, pc.Reconstruction]:
    return {int(d): pc.Reconstruction(os.path.join(out_dir, d)) for d in sorted(os.listdir(out_dir)) if d.isdigit()}


def image_poses(models: dict[int, pc.Reconstruction], names: dict) -> dict[tuple[str, float], pc.Rigid3d]:
    """``{(camera, time): cam_from_world}`` over every posed image in every model (all in INS ENU)."""
    return {names[im.name]: im.cam_from_world() for r in models.values() for im in r.images.values() if im.has_pose}


def robust_mean(rotations: Rotation, translations: np.ndarray, cluster_deg: float = 1.0) -> tuple[Rotation, np.ndarray, np.ndarray, np.ndarray]:
    ...
    mean = rotations[keep].mean()
    return mean, np.median(translations[keep], 0), np.degrees((mean.inv() * rotations).magnitude()), keep


def derive_rig(models: dict[int, pc.Reconstruction], names: dict, reference: str) -> dict[str, dict]:
    ...
    rig = {}
    for camera in sorted({c for c, _ in names.values()}):
        rel = [poses[(camera, t)] * poses[(reference, t)].inverse() for (c, t) in poses if c == camera and (reference, t) in poses]
        if len(rel) < 3:
            raise RuntimeError(f"{camera}: only {len(rel)} frames shared with {reference}; cannot initialise the rig")
        rot, trans, angles, keep = robust_mean(Rotation.from_quat([x.rotation.quat for x in rel]), np.array([x.translation for x in rel]))
        rig[camera] = {"cam_from_rig": pc.Rigid3d(pc.Rotation3d(rot.as_quat()), trans), "frames": int(keep.sum()),
                       "rotation_scatter_deg": float(np.median(angles[keep])), "translation_std_m": np.array([x.translation for x in rel])[keep].std(0)}
    return rig


def model_cameras(models: dict[int, pc.Reconstruction], names: dict) -> dict[str, pc.Camera]:
...
def rigged_model(db_path: str, models: dict[int, pc.Reconstruction], names: dict, reference: str, out_dir: str) -> pc.Reconstruction:
    ...
    config = pc.RigConfig(cameras=[
        pc.RigConfigCamera(ref_sensor=(name == reference), image_prefix=name + "/", camera=cameras[name],
