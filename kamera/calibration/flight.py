"""Flight discovery: synchronized frames, camera naming, INS trajectory, COLMAP image tree."""
...
# NED body attitude -> ENU: swap north/east and flip down (a 180 deg turn about (1,1,0)).
NED_TO_ENU = Rotation.from_quat([np.sqrt(0.5), np.sqrt(0.5), 0.0, 0.0])


@dataclass
class Frame:
    """All images captured on one trigger event, keyed by camera name (e.g. ``C_rgb``)."""
...
class InsTrajectory:
    ...
    Quaternions follow the KAMERA convention: ``rotation`` maps body (forward, right, down)
    vectors into the local ENU frame. Any source with times, lat/lon/alt and heading/pitch/roll
    can build one, so a future high-rate or event-stamped log drops in via ``__init__``.
    """
...
def discover_flight(flight_dir: str) -> tuple[list[Frame], InsTrajectory, str]:
    """Group every ``*_meta.json`` under ``flight_dir`` into trigger-synchronized frames.
    ...
    trajectory assembled from the per-image INS samples, and the rig name from ``sys_cfg``.
    """
...
def normalize(src: str, dst: str) -> None:
    """Percentile-stretch (0.1-99.9) a dim or 16-bit frame to 8 bits and apply CLAHE, so SIFT has contrast."""
...
def build_image_tree(
    ...
    time-based names. RGB is symlinked; the dim UV and 16-bit IR frames are contrast-normalized.
    ...
    for frame in frames:
        for camera, src in frame.images.items():
            stretch = not camera.endswith("_rgb")
